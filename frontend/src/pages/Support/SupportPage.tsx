...
export default function SupportPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!firstName.trim()) { setErrorMsg("Введіть ім'я."); return; }
    if (!subject)           { setErrorMsg('Виберіть тему звернення.'); return; }
    if (!message.trim())    { setErrorMsg('Напишіть повідомлення.'); return; }
    ...
    if (error) {
      setErrorMsg('Помилка відправки. Спробуйте ще раз.');
  ...
  const globalStyles = `
    ...
    select option { background-color: #050506; color: #9E9E9E; }
  `;
    ...
    <section className="text-[#FFFFFF] w-full mx-auto bg-transparent font-['Montserrat',sans-serif]">
      ...
      <div className="pt-[24px] pl-[40px] pr-[40px] pb-[24px] flex justify-start gap-[24px]">
        ...
        >
          ...
          {submitState === 'success' ? (
            <div className="flex flex-col items-center justify-center flex-1 gap-4 px-[24px]">
              <div className="w-14 h-14 rounded-full bg-[#8348C1]/20 border border-[#8348C1]/40 flex items-center justify-center">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#C38BFF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="20 6 9 17 4 12"/>
                </svg>
              </div>
              <p className="text-[18px] font-semibold text-white text-center">Звернення надіслано!</p>
              <p className="text-[14px] text-[#A3A4B0] text-center leading-relaxed">
                Ми отримали ваше повідомлення і відповімо якнайшвидше.
              </p>
              <button
                onClick={() => setSubmitState('idle')}
                className="mt-2 px-6 h-[44px] rounded-[28px] text-[14px] font-medium text-[#C38BFF] border border-[#8348C1]/40 hover:bg-[#8348C1]/10 transition-colors"
              >
                Надіслати ще одне
              </button>
            </div>
                ...
                {errorMsg && (
                  <div className="px-[24px] mb-[12px]">
                    <p className="text-[12px] text-red-400 flex items-center gap-1.5">
                      <span className="inline-flex w-4 h-4 items-center justify-center rounded-full border border-red-400/50 text-[9px]">!</span>
                      {errorMsg}
                    </p>
                  </div>
                )}
