...
const RegisterPage: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (!acceptedTerms) {
      errors.terms = 'Підтвердіть згоду з умовами користування';
    }
  ...
  const getInputClass = (hasError?: boolean) => {
    return [
      'w-full h-[40px] px-5 bg-transparent rounded-full text-[13px] text-white outline-none placeholder:text-[#A3A4B0]/50 transition-all duration-200',
      hasError
  ...
  const getPasswordInputClass = (hasError?: boolean) => {
    return [
      'w-full h-[40px] px-5 pr-12 bg-transparent rounded-full text-[13px] text-white outline-none placeholder:text-[#A3A4B0]/50 transition-all duration-200 font-sans',
      hasError
        ? 'border border-red-400/60 focus:border-red-400 shadow-[0_0_0_3px_rgba(248,113,113,0.08),0_0_18px_rgba(248,113,113,0.12)]'
        : 'border border-white/10 focus:border-[#8348C1] focus:shadow-[0_0_0_3px_rgba(131,72,193,0.10)]',
    ].join(' ');
    ...
    <div className="min-h-screen bg-[#010004] flex flex-col relative overflow-hidden font-montserrat">
      ...
      <div className="flex-1 flex items-center justify-center z-10 p-4">
        <div className="p-[1px] rounded-[30px] bg-[linear-gradient(90deg,rgba(82,46,139,0.32),rgba(179,179,179,0.32))] w-full max-w-[480px]">
          <div className="relative h-full rounded-[29px] bg-[#050506] px-8 md:px-10 py-8 md:py-10 text-center shadow-[0_20px_70px_rgba(131,72,193,0.10),0_8px_25px_rgba(0,0,0,0.35)] overflow-hidden">
              ...
              <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-3 text-left">
                ...
                <div className="flex flex-col gap-1 relative">
                  ...
                  <button
                    ...
                    className="absolute right-4 top-[24px] text-[#A3A4B0] hover:text-white transition-colors"
                  >
                ...
                <div className="mt-1">
                  <label className="flex cursor-pointer items-start gap-3 rounded-[18px] border border-white/10 bg-white/[0.02] px-4 py-3 transition-colors hover:bg-white/[0.04]">
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className={[
                        'mt-[1px] flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-[6px] border transition-all',
                        acceptedTerms
                          ? 'border-[#8348C1] bg-[#8348C1] shadow-[0_0_14px_rgba(131,72,193,0.35)]'
                          : fieldErrors.terms
                    ...
                    <span className="text-[11px] leading-[17px] text-[#A3A4B0]">
                      Я погоджуюсь з{' '}
                      <Link to="/terms" className="text-[#C38BFF] hover:underline">
                        Умовами користування
                      </Link>{' '}
                      та{' '}
                      <Link to="/privacy" className="text-[#C38BFF] hover:underline">
                        Політикою конфіденційності
                      </Link>
                    </span>
                  </label>

                  <ErrorMessage message={fieldErrors.terms} />
                </div>

                <button
                  type="submit"
                  disabled={loading || checkingEmail}
                  className="w-full h-[40px] mt-1 rounded-full bg-gradient-to-r from-[#2C1969] via-[#8348C1] to-[#C38BFF] text-white text-[14px] font-medium transition-transform hover:scale-105 active:scale-95 disabled:opacity-60 disabled:hover:scale-100 shadow-[0_4px_15px_rgba(131,72,193,0.2)]"
                >
                  {loading ? 'Створення...' : 'Створити акаунт'}
                </button>
              </form>

              <div className="flex items-center gap-4 my-5 opacity-90">
                <div className="h-[1px] flex-1 bg-[linear-gradient(90deg,#000000_0%,#8348C1_48%,#2C1969_100%)]"></div>
                <span className="text-[11px] text-[#A3A4B0]">або</span>
                <div className="h-[1px] flex-1 bg-[linear-gradient(270deg,#000000_0%,#8348C1_48%,#2C1969_100%)]"></div>
              </div>

              <div className="flex flex-col gap-2">
                <button
                  type="button"
                  className="w-full h-[40px] rounded-full border border-white/10 flex items-center justify-center gap-3 text-[12px] text-white hover:bg-white/5 transition-colors"
                >
      ...
      <div className="w-full text-center pb-6 z-10 relative">
